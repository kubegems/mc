...
// alias = https://minio:minio@123:token@@localhost:9000
/*
	accessKey:    "minio",
	secretKey:    "minio@123",
	sessionToken: "token@",
	...
	port:         "9000",
*/
// alias1 = https://minio:minio1#23@localhost:9000
/*
	accessKey: "minio",
	secretKey: "minio1#23",
	hostname:  "localhost",
...
const (
	...
	mcEnvAliasSrc   = "MC_ALIAS_SRC"
	mcEnvAliasTgt   = "MC_ALIAS_TGT"
)
...
// 专门针对PAI中拷贝任务设置的环境变量
func readAliasFromMap(alias map[string]string) *probe.Error {
	for ali, envURL := range alias {
		if envURL == "" {
			return probe.NewError(fmt.Errorf("value is empty")).Trace(ali)
		}
		aliasConfig, err := expandAliasFromEnv(envURL)
		if err != nil {
			return err.Trace(envURL)
		}
		aliasToConfigMap[ali] = aliasConfig
	}
	return nil
}

//MC_ALIAS_SRC=https://minio:minio1#23@localhost:9000
//MC_ALIAS_TGT=https://minio:minio1#23@localhost:9000
