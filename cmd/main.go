...
// export MC_ALIAS_SRC=https://minio:minio1#23@localhost:9000
// export MC_ALIAS_TGT=https://minio:minio1#23@localhost:9000
// mc mirror pai-src/xxxx pai-tgt/xxxx
func init() {
	...
	if env.IsSet(mcEnvAliasSrc) && env.IsSet(mcEnvAliasTgt) {
		//专门针对PAI中拷贝任务设置的环境变量
		src := env.Get(mcEnvAliasSrc, "")
		tgt := env.Get(mcEnvAliasTgt, "")
		alias := map[string]string{
			"pai-src": src,
			"pai-tgt": tgt,
		}
		fatalIf(readAliasFromMap(alias).Trace("pai-alias"), "Unable to parse pai-alias")
	}
